    ...
    return (
        ...
        >
            {data.map((item, index) => {
                ...
                return <Tab key={index} disabled={Boolean(item.disabled)} className="normal-case" label={item.name} value={item.id} icon={item.otherComp ? item.otherComp : (item.icon && <Icon>{item.icon}</Icon>)} />
            })}
        </Tabs>
    )
}
